...
func (h *harness) createWithInputs(name string, layers []v1alpha1.InputSource) *v1alpha1.Weave {
	h.t.Helper()
	w := &v1alpha1.Weave{
...
func inlineValues(raw string) v1alpha1.InputSource {
...
func TestInputsLayerConfigMapUnderInlineValues(t *testing.T) {
	h := newHarness(t, nil)
	...
	h.createWithInputs("layered", []v1alpha1.InputSource{
		{ConfigMap: &v1alpha1.InputRef{Name: "platform"}},
	...
	for k, want := range map[string]string{
		...
		"tier":   "premium", // the later layer wins
...
func TestInputsFromSecretAreDecoded(t *testing.T) {
	h := newHarness(t, nil)
	...
	h.createWithInputs("fromsecret", []v1alpha1.InputSource{
		{Secret: &v1alpha1.InputRef{Name: "creds"}},
	})
...
func TestInputsFromAKeyParsedAsYAML(t *testing.T) {
	h := newHarness(t, nil)
	h.configMap("values", map[string]string{
		"values.yaml": "image:\n  repository: base\n  tag: v1\nreplicas: 2\n",
	})

	h.createWithInputs("document", []v1alpha1.InputSource{
		{ConfigMap: &v1alpha1.InputRef{Name: "values", Key: "values.yaml"}},
	...
	// The document was parsed into a mapping, and the inline layer merged into
...
// failure.
func TestInputsWaitForAMissingObject(t *testing.T) {
	h := newHarness(t, nil)

	h.createWithInputs("waiting", []v1alpha1.InputSource{
		{ConfigMap: &v1alpha1.InputRef{Name: "not-there"}},
	})
	h.settle("waiting", 2)

	c := requireCondition(t, h.weave("waiting"), naming.ConditionWaiting, metav1.ConditionTrue)
	if c.Reason != ReasonInputMissing {
		t.Errorf("reason = %q, want %q", c.Reason, ReasonInputMissing)
	}
	if h.exists("echo") {
...
// Optional layers are for configuration that may legitimately not be there.
func TestInputsSkipAnOptionalMissingObject(t *testing.T) {
	h := newHarness(t, nil)

	h.createWithInputs("optional", []v1alpha1.InputSource{
		inlineValues(`{"base": "yes"}`),
		{ConfigMap: &v1alpha1.InputRef{Name: "absent", Optional: true}},
	})
	h.settle("optional", 2)
...
// the Weave.
func TestInputsReactToAConfigMapChange(t *testing.T) {
	h := newHarness(t, nil)
	h.configMap("settings", map[string]string{"mode": "before"})

	h.createWithInputs("reactive", []v1alpha1.InputSource{
		{ConfigMap: &v1alpha1.InputRef{Name: "settings"}},
	})
	h.settle("reactive", 2)
...
func TestInputsAreReadUnderImpersonation(t *testing.T) {
	h := newHarness(t, nil)
	...
	w := &v1alpha1.Weave{
		...
		Spec: v1alpha1.WeaveSpec{
			...
			Inputs: []v1alpha1.InputSource{
				{ConfigMap: &v1alpha1.InputRef{Name: "secrets-of-state"}},
			},
			Program: echoInputs,
		},
	}
